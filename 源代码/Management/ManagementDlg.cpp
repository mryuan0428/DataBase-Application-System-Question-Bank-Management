	...
	: CDialog(CManagementDlg::IDD, pParent)
{
	...
	//初始化数据库
	CString strConnection="File Name=my_data1.udl";
...
void CManagementDlg::OnButtonLogin() 
{
	...
	{
		UpdateData(TRUE);
		strUsername = loginDlg.GetUsername();
		strPurview = loginDlg.GetPurview();
		strSubject = loginDlg.GetSubject();
		CManageAllDlg manageallDlg;
		manageallDlg.DoModal();
	}
	else
		OnCancel();
	ShowWindow(SW_SHOW);
